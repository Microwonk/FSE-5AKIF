...
class ExceptionRestController {
    ...
    @ExceptionHandler
    public ResponseEntity<ApiErrorResponse> handle(OrderInitiateDeliveryException orderInitiateDeliveryException) {
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "Failed to prepare order for delivery ...");
        return ResponseEntity.badRequest().body(new ApiErrorResponse("7000", orderInitiateDeliveryException.getMessage()));
    }
